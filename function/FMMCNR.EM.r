...
FMMCNR.EM = function(Y, X, g, distr=c('MVN','MCN'), true.clus=NULL, tol=1e-5, max.iter=1000, per=10, Beta.true=NULL, SE=FALSE)
{
    require(mvtnorm)
    begin = proc.time()[1]
    n = nrow(Y)
...
# old observed-data log-likelihood
    for(i in 1: g){
     ...
     if(distr=='MCN'){
         for(j in 1: n) w.pdf[j, i] = nu[i] * dmvnorm(t(Y[j, ]), mean=mu[,j,i], sigma=round(Sigma[,,i]/rho[i],6)) + (1-nu[i]) * dmvnorm(t(Y[j, ]), mean=mu[,j,i], sigma=round(Sigma[,,i],6)) 
     }               
...
#E-step
      for(i in 1: g){
        if(distr=='MCN'){
          for(j in 1: n){
            dcn = dmni(c(Y[j, ]), mu=mu[,j,i], Sigma=round(Sigma[,,i],6), nu=nu[i], rho=rho[i], distr='MCN')
            w.rho[j, i] = nu[i] * dmvnorm(t(c(Y[j, ])), mean=mu[,j,i], sigma=round(Sigma[,,i]/rho[i], 6)) / dcn 
            w1[j, i] = (1-nu[i]) * dmvnorm(t(c(Y[j, ])), mean=mu[,j,i], sigma=round(Sigma[,,i],6)) / dcn
          }
...
# M-step
      ...
      for(i in 1: g){
        ...
        Beta[,i] = solve(t(Tga * X) %*% TSig.inv %*% X) %*% (t(Ta * X) %*% TSig.inv %*% (Tga*y))
        vec.mu = X %*% Beta[,i]
        mu[,,i] = matrix(vec.mu, nrow=p, ncol=n)
        cent[,,i] = t(Y) - mu[,,i]
...
# clustering
 ...
 # imputation
    ...
    cat(paste(rep("-", 50), collapse = ""), "\n")
    if(SE == TRUE){
     IM = I.FMMCNR(para, Y=Y, X=X, distr=distr, EST=EST, wden=wden)
     return(list(model.inf = model.inf, para = para, EST = EST, IM = IM, pre.cls = pre.cls, xi = xi, yfit = yfit))
    } else{
     return(list(model.inf = model.inf, para = para, EST = EST, pre.cls = pre.cls, xi = xi, yfit = yfit))
    }
}

# Louis' information matrix
I.FMMCNR = function(para.est, Y, X, distr=c('MVN','MCN'), EST, wden)
{
...
# individual expected score vector
  ...
  for(i in 1: g){
     ...
     for(j in 1: n){
      ...
      EIs[(aa[i]+1): aa[(i+1)], ][2:(1+ss), j] = (a.hat[j, i] * tau[j, i] * t(X[idx, ])) %*% Sig.inv[,,i] %*% (Y[j,] - mu[,j,i]) 
      A1 = (tau[j,i] * cent[, j,i]) %*% t(cent[, j,i])
      A2 = (w.rho[j,i] * cent[, j,i]) %*% t(cent[, j,i])
     for(k in 1: q1){
      SSdot = Sig.inv[,,i] %*% dot.Sig[,,k]
      EIs[(aa[i]+1): aa[(i+1)], ][(1+ss+k), j] = 0.5 * a.hat[j,i] * (tr(A1 %*% SSdot %*% Sig.inv[,,i]) - tr(SSdot))
     }
     if(distr == 'MCN') EIs[(aa[i]+1): aa[(i+1)], ][(3+ss+q1), j] = 0.5 * a.hat[j,i] * (p*w.rho[j,i]/rho[i] - tr(A2 %*% Sig.inv[,,i]))
    }
    if(distr == 'MCN') EIs[(aa[i]+1): aa[(i+1)], ][(2+ss+q1), ] = a.hat[,i] * (w.rho[,i] - nu[i]) / (nu[i] * (1-nu[i]))
  }
