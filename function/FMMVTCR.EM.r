...
rfmmvt.reg = function(n, para, Xi, distr=c('MVT','MVN'))
{
  distr = distr[1]
  w = para$w
  Beta = para$Beta 
...
FMMVTCR.EM = function(Yc, X, cen, g, distr=c('MVN','MVT'), true.clus=NULL, tol=1e-6, max.iter = 1000, per=10, Beta.true=NULL, SE=FALSE)
{
  require(mvtnorm)
        ...
        if(pjc == 0){ 
          ...
          if(pjc == p){
            ...
            if(distr=='MVT'){
              for(s in 1: no.ind){
               yjc = c(Cj%*%Yc[ind[s], ])
               tau[ind[s], i] = ptmvt(lowerx=rep(-Inf, pjc), upperx=yjc, mean=mu.co[,s], sigma=round(nu[i]/(nu[i]+2)*Sigma[,,i],6), df=round(nu[i]+2))/ptmvt(lowerx=rep(-Inf, pjc), upperx=yjc, mean=mu.co[,s], sigma=round(Sigma[,,i],6), df=round(nu[i]))
               EX = TMNI.moment(mu=mu.co[,s], Sigma=round(nu[i]/(nu[i]+2)*Sigma[,,i],6), nu=nu[i]+2, distr='MVT', a.low=rep(-Inf, pjc), a.upp=yjc)
               yc.hat[,s,i] = EX$EY
            ...
            if(distr=='MVT'){
              ...
              for(s in 1: no.ind){
               ...
               } else cdf.rate = ptmvt(lowerx=rep(-Inf, pjc), upperx=yjc, mean=c(mu.co[,s]), sigma=rep((DF/(DF+2)),pjc)*newScc.o, df=DF+2) / ptmvt(lowerx=rep(-Inf, pjc), upperx=yjc, mean=c(mu.co[,s]), sigma=round(newScc.o,6), df=DF)
...
# M-step
     for(i in 1: g){ 
      ...
      if(distr=='MVT'){
        nu[i] = nlminb(start = nu[i], objective = FMMVTCR.nu.fn, lower = 2+1e-6, upper =Inf, gup=i, w=w, Beta=Beta, Sigma=Sigma, nu=nu, Yc=Yc, X=X, cen=cen)$par     
      }}
...
# clustering
 ...
 # imputation
    ...
    cat(paste(rep("-", 50), collapse = ""), "\n")
    if(SE == TRUE){
     IM = I.FMMVTCR(para.est, Yc, X, cen=cen, distr=distr, EST=EST, wden=wden)
...
FMMVTCR.nu.fn = function(par, gup, w, Beta, Sigma, nu, Yc, X, cen)
{
  nu[gup] = par
  n = nrow(Yc)
  p = ncol(Yc)
...
I.FMMVTCR = function(para.est, Yc, X, cen, distr=c('MVN','MVT'), EST, wden)
{
  GB = GenzBretz(maxpts = 5e4, abseps = 1e-9, releps = 0)
  n = nrow(Yc)
  p = ncol(Yc)
     ...
     if(pjc == 0){ 
       ...
       if(pjc == p){
         ...
         if(distr=='MVT'){
           for(s in 1: no.ind){
            yjc = c(Cj%*%Yc[ind[s], ])
            tau[ind[s], i] = ptmvt(lowerx=rep(-Inf, pjc), upperx=yjc, mean=mu.co[,s], sigma=nu[i]/(nu[i]+2)*Sigma[,,i], df=round(nu[i]+2))/ptmvt(lowerx=rep(-Inf, pjc), upperx=yjc, mean=mu.co[,s], sigma=Sigma[,,i], df=round(nu[i]))
...
# Meilijson (1989) formula
   ...
   if(distr == 'MVT'){                              
    sel = sel1 = c(1,m1)
    if(g >= 2) for(i in 1:(g-1)) sel = c(sel, sel1+(m1*i)) 
    sd.theta[sel] = sqrt(diag(solve(I.theta[sel, sel])))
    sd.theta[-sel] = sqrt(diag(solve(I.theta[-sel, -sel])))
   } 
   if(distr == 'MVN'){
