...
#Generate MCNR data
rfmmcn.reg = function(n, para, Xi, distr=c('MCN','MVN'))
{
  distr = distr[1]
  w = para$w
  Beta = para$Beta 
  ...
  for(i in 1: g){
   ...
   p = nrow(Xi)
   Y = rbind(Y, matrix(rep(Xi %*% Beta[,i], N[i]), nrow=N[i], byrow=T) + 1/sqrt(tau) * rmvnorm(N[i], mean=rep(0,p), sigma=Sigma[,,i]))
   Tau = c(Tau, tau)
  } 
...
FMMCNCR.EM = function(Yc, X, cen, g, distr=c('MVN','MCN'), true.clus=NULL, tol=1e-6, max.iter = 1000, per=10, Beta.true=NULL, SE=FALSE)
{
  require(mvtnorm)
...
# fix label switch
  if(length(Beta.true) != 0){
     allper = permn(g) 
     pk = factorial(g)
     beta.norm = length(pk)
     for(k in 1: pk) beta.norm[k] = sum((Beta[,allper[[k]]] - Beta.true)^2)
     cho = order(beta.norm)[1]
     w = w[allper[[cho]]]
     Beta = Beta[, allper[[cho]]]
...
# old observed-data log-likelihood
  for(i in 1: g){
   ...
   for(j in 1:num.class.cen){
    ...
    if(distr=='MCN'){
      if(pjc == 0){
        for(s in 1: no.ind) w.cdf.pdf[ind[s], i] = nu[i] * dmvnorm(t(Yc[ind[s], ]), mean=mu[, ind[s], i], sigma=round(Sigma[,,i]/rho[i],6)) + (1-nu[i]) * dmvnorm(t(Yc[ind[s], ]), mean=mu[, ind[s], i], sigma=round(Sigma[,,i], 6)) 
      }
      if(pjc == 1){
        for(s in 1: no.ind){
        fyo = dmni(c(O%*%Yc[ind[s], ]), mu=c(O%*%mu[, ind[s], i]), Sigma=round(OSO, 6), nu=nu[i], rho=rho[i], distr='MCN') 
        nu2.1 = nu[i] * dmvnorm(t(c(O%*%Yc[ind[s], ])), mean=c(O%*%mu[, ind[s], i]), sigma=round(OSO/rho[i], 6)) / fyo
      ...
      if(pjc == (p-1)){
        for(s in 1: no.ind){ 
        fyo = dmni(c(O%*%Yc[ind[s], ]), mu=c(O%*%mu[, ind[s], i]), Sigma=round(OSO, 6), nu=nu[i], rho=rho[i], distr='MCN')
        nu2.1 = nu[i] * dnorm(c(O%*%Yc[ind[s], ]), mean=c(O%*%mu[, ind[s], i]), sd=sqrt(OSO/rho[i])) / fyo
        w.cdf.pdf[ind[s], i] = (nu2.1 * ptmvnorm(lowerx=rep(-Inf,pjc), upperx=c(Cj%*%Yc[ind[s], ]), mean=c(mu.co[,s]), sigma=round(Scc.o/rho[i],6)) + (1-nu2.1) * ptmvnorm(lowerx=rep(-Inf,pjc), upperx=c(Cj%*%Yc[ind[s], ]), mean=c(mu.co[,s]), sigma=round(Scc.o, 6))) * fyo  
      }} 
      if(pjc == p){
    ...
    cat("EM is running for the ", g, "-component FM-MR model with ", distr, "distribution: censoring proportion ", mean(cen)*100, '%.', "\n")
       ...
       for(j in 1:num.class.cen){
        ...
        if(pjc == 0){ 
          ...
          if(distr=='MCN'){
            ...
            for(s in 1: no.ind){
              dcn = dmni(c(Yc[ind[s], ]), mu=mu[, ind[s], i], Sigma=round(Sigma[,,i], 6), nu=nu[i], rho=rho[i], distr='MCN')
              w.rho[s] = nu[i] * dmvnorm(t(c(Yc[ind[s], ])), mean=mu[, ind[s], i], sigma=round(Sigma[,,i]/rho[i], 6)) / dcn 
              w1[s] = (1-nu[i]) * dmvnorm(t(c(Yc[ind[s], ])), mean=mu[, ind[s], i], sigma=round(Sigma[,,i],6)) / dcn
            }
            xi[ind, i] = w.rho 
          }
          yhat[ind, , i] = Yc[ind, ]
...
# clustering
 ...
 # imputation
    ...
    cat(paste(rep("-", 50), collapse = ""), "\n")
    if(SE == TRUE){
      IM = I.FMMCNCR(para, Yc=Yc, X=X, cen=cen, distr=distr, EST=EST, wden=wden)
...
I.FMMCNCR = function(para.est, Yc, X, cen, distr=c('MVN','MCN'), EST, wden)
{
  GB = GenzBretz(maxpts = 5e4, abseps = 1e-9, releps = 0)
  n = nrow(Yc)
  p = ncol(Yc)
